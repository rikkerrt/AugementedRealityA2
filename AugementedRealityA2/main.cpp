...
void update()
{
    ...
    if (car->position.x == startLine.x && car->position.z == startLine.z)
    {
	...
	if (car->position.x == endLine.x && car->position.z == endLine.z)
	{
...
void draw()
{
    ...
    glm::mat4 ortho = glm::ortho(0.0f, (float)viewport[2], (float)viewport[3], 0.0f);
